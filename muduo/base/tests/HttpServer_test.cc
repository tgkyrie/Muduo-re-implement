...
#include "muduo/base/JsonValue.h"
...
using namespace muduo::json;

std::string rootDir="/home/tgkyrie/project/muduo_reimple/";
std::map<std::string,std::string> gFileMap;
...
std::string loadFile(const std::string& fileName){
  std::ifstream inFile;
    ...
    std::string buffer;
    buffer.assign(std::istreambuf_iterator<char>(inFile),
...
void preLoadFile(){
  for(std::string& fileName:gFileNames){
    std::ifstream inFile;
    inFile.open(fileName,std::ios::in);
    std::string buffer;
    buffer.assign(std::istreambuf_iterator<char>(inFile),
      std::istreambuf_iterator<char>());
    gFileMap[fileName]=std::move(buffer);
  }
}
...
//   std::cout << "Headers " << req.methodString() << " " << req.path() << std::endl;
  ...
  if(req.method()==HttpRequest::kGet){
    if (req.path() == "/HelloWorld")
    {
        resp->setHttpStatusCode(HttpResponse::k200Ok);
    ...
    {
        resp->setHttpStatusCode(HttpResponse::k404NotFound);
    ...
    if(req.path()=="/loopback"){
        std::cout<<"[post requst]"<<std::endl;
        JsonValue jvalue;
        JsonValue responseJson;
        resp->setContentType("application/json");
        resp->addHeader("Server","Muduo");
        jvalue.fromString(req.body());
        bool correct=false;
        if(jvalue.get("username")!=nullptr&&jvalue.get("password")!=nullptr){
          std::string username=jvalue.get("username")->toStr();
          std::string password=jvalue.get("password")->toStr();
          correct=login(username,password);
        }
        if(correct){
          resp->setHttpStatusCode(HttpResponse::k200Ok);
          resp->setStatusString("OK");
          responseJson.insert(200,"code");
          resp->setBody(responseJson.serialize());
        }
        else {
          resp->setHttpStatusCode(HttpResponse::k200Ok);
          resp->setStatusString("No Content");
          responseJson.insert(204,"code");
          responseJson.insert("用户名或密码错误","message");
          resp->setBody(responseJson.serialize());
        }
